...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['order_code'])) {
    ...
    if (!$order || $order['status'] !== 'Đang chờ quét QR') {
        die('Đơn hàng đã được xử lý hoặc không hợp lệ.');
    }
    $orderId = (int)$order['id'];

    // 2. Lấy danh sách order_products
    $sql = "
      SELECT 
      ...
      WHERE op.order_id       = ?
        ...
        AND p.khoa_tem         = 0
      GROUP BY
        op.id, op.product_name, op.quantity,
        op.is_promotion, p.nhap_tay, p.snnew
...
async function processScanned(raw) {
  ...
  // --- MODE 0: Lookup full serial và tìm dòng phù hợp mode=0
  const res0 = await lookupSerial(rawSer);
  if (res0 && res0.product_name) {
    const items = Array.from(document.querySelectorAll('.product-item[data-nhap-tay="0"]'));
    for (let item of items) {
      if ((item.dataset.productName || '').trim() === res0.product_name.trim()) {
        fullSn = rawSer;
        productName = res0.product_name.trim();
        matchedMode = 0;
        break;
      }
    }
  ...
  // --- MODE 2: Nếu không khớp mode=0 thì thử prefix+last3
  if (!fullSn && rawSer.length >= 3) {
    ...
    for (let item of items) {
      ...
      if (res2 && res2.product_name) {
        if ((item.dataset.productName || '').trim() === res2.product_name.trim()) {
          fullSn = candidate;
          productName = res2.product_name.trim();
          matchedMode = 2;
          break;
        }
      }
  ...
  // --- MODE 1: Nếu không tìm được SN phù hợp, kiểm tra có dòng nhập tay không
  if (!fullSn) {
  ...
  // --- Gán SN vào ô trống trong dòng phù hợp
  const targets = Array.from(
    document.querySelectorAll(`.product-item[data-product-name="${productName}"][data-nhap-tay="${matchedMode}"]`)
  );
  for (let item of targets) {
    ...
    if (empty) {
      ...
      displayMessage(`✅ Đã thêm SN "${fullSn}" cho "${productName}".`);
      scanSound.play();
      // ✅ Cuộn dòng hiện tại vào giữa màn hình
  ...
  displayMessage(`❌ Đã quét đủ SN cho "${productName}".`, 'error');
  scanSoundError.play();
  busy = false;
}
  // 7. Khởi động html5-qrcode, bắt buộc facingMode = environment
...
// Thay vì getCameras(), trực tiếp yêu cầu rear camera:
html5QrCode.start(
  // cameraConstraints:
  ...
  // Nếu thất bại (ví dụ iOS không hỗ trợ exact), fallback về any:
  html5QrCode.start(
