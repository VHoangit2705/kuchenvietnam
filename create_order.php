...
function labelOrderType($t)
{
  switch (strtolower(trim((string)$t))) {
    case 'outside':
      return 'Tại kho';
    case 'droppii':
      return 'DROPPII VIETTEL';
    case 'warehouse_viettel':
      return 'KHÁCH LẺ VIETTEL';
    case 'warehouse_ghtk':
      return 'KHÁCH LẺ GHTK';
    case 'warehouse_branch':
      return 'KHÁCH LẺ QUA KHO';
    default:
      return $t;
  }
}
...
switch ($position) {
  case 'Đơn hàng Vinh':
    $warehouseLabel = 'Giao hàng từ kho Vinh';
    break;
  case 'Đơn hàng HaNoi':
    $warehouseLabel = 'Giao hàng từ kho HaNoi';
    break;
  case 'Đơn hàng HCM':
    $warehouseLabel = 'Giao hàng từ kho HCM';
    break;
  default:
    $warehouseLabel = 'Giao hàng từ kho ADMIN';
    break;
}
...
<head>
  ...
  <style>
    ...
    .badge-new {
      ...
      font-weight: 700;
      letter-spacing: .5px;
      animation: neonBlink .9s ease-in-out infinite;
      border-radius: .35rem;
    }
...
<body>
                  ...
                  <tbody>
                    <?php if ($hist): foreach ($hist as $h): ?>
                        <tr>
                          ...
                          <td><?= esc($h['customer_name']) ?><div class="mini-muted"><?= esc($h['customer_phone']) ?></div>
                          </td>
  ...
  <script>
    ...
    function productOptionsHtml(orderType = null) {
      const list = orderType ? sortProductsByOrderType(orderType) : PRODUCTS;
  ...
  `).join('');
    ...
    function sortProductsByOrderType(orderType) {
      const priorityView = ORDER_VIEW_PRIORITY[orderType];
      if (!priorityView) return PRODUCTS;

      return [...PRODUCTS].sort((a, b) => {
        const av = Number(a.view || 0);
        const bv = Number(b.view || 0);

        if (av === priorityView && bv !== priorityView) return -1;
        if (av !== priorityView && bv === priorityView) return 1;
        return 0; // giữ nguyên thứ tự backend
      });
    }
    ...
    const ORDER_VIEW_PRIORITY = {
      hurom: 3,
      bohemia: 5,
      omada: 6
    };

    async function generateUniqueOrderCode(prefix = 'KU') {
      const letters = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
      while (true) {
        const t = Date.now().toString().slice(-6);
        const code = `${prefix}${letters[Math.floor(Math.random()*letters.length)]}${t}`;
  ...
  `;
    ...
    function addProduct(orderNumber) {
      ...
      const mode = document.getElementById(`order_mode_${orderNumber}`)?.value || '';
      const html = `
...
<input type="hidden"
       class="product-id"
       name="orders[${orderNumber}][products][${nextIndex}][product_id]">

<input type="hidden"
       class="product-name"
       name="orders[${orderNumber}][products][${nextIndex}][product_name]">
  ...
  `;
    ...
    function togglePromotionFields(card) {
      ...
      if (isPromo) {
        groupDiff.classList.remove('d-none');
        g1.classList.add('d-none');
        g2.classList.add('d-none');
      } else {
        groupDiff.classList.add('d-none');
    ...
    function computeProductRow(card) {
      ...
      const hidId = card.querySelector('.product-id');
      const hidName = card.querySelector('.product-name');

      if (sel && sel.selectedOptions[0]) {
        hidId.value = sel.selectedOptions[0].dataset.id || '';
        hidName.value = sel.value || '';
      }
    ...
    function ensureSelect2(sel, cfg) {
      ...
      $(sel)
        .on('select2:open', () => {
          if (DG_DEBUG) console.log('[DG][open]', sel.id);
        })
        .on('select2:close', () => {
          if (DG_DEBUG) console.log('[DG][close]', sel.id);
        })
        .on('change', () => {
          if (DG_DEBUG) console.log('[DG][change]', sel.id, '→', $(sel).val());
        });
    }
    ...
    async function fetchProvincesOnce() {
      ...
      const r = await fetch(API_PROVINCES, {
        credentials: 'same-origin'
      });
    ...
    async function fetchDistrictsByProvince(pid) {
      ...
      const r = await fetch(API_DISTRICTS(pid), {
        credentials: 'same-origin'
      });
      const j = await r.json();
      console.log('success=', j?.success, 'items=', j?.data?.length);
    ...
    async function fetchWardsByDistrict(did) {
      ...
      const r = await fetch(API_WARDS(did), {
        credentials: 'same-origin'
      });
      const j = await r.json();
      console.log('success=', j?.success, 'items=', j?.data?.length);
    ...
    async function initProvinceSelect(order, prePid = 0, preDid = '', preWid = '') {
      ...
      console.log('[DG][init] order=', order, 'pre=', {
        prePid,
        preDid,
        preWid
      });
      ...
      async function onDistrictChanged() {
        ...
        if (!did || !/^\d+$/.test(String(did)) || did === lastDid) {
          lastDid = did;
          return;
        }
        lastDid = did;
      ...
      function onWardsChanged() {
        ...
        dgg('%c[DG][Wards change]', 'color:#e67700', 'wid=', wid, 'text=', text);
        dge();
      }
      ...
      if (prePid) {
        ...
        try {
          ...
          console.error('[DG][preselect] error', err);
        }
      }
    }
    ...
    async function fetchAddressSuggest(query) {
      try {
        ...
        console.error("[TrackAsia] error", e);
      }
      return null;
    }
    ...
    function applyCustomerDataToForm(order, c) {
      ...
      if (pid || did || wid) {
        dlog(DEBUG_AUTOFILL, '[applyCustomerDataToForm] initProvinceSelect with', {
          pid,
          did,
          wid
        });
    ...
    function handleCustomerPhoneAutoFill(order) {
      ...
      input.addEventListener('blur', async () => {
        ...
        try {
          ...
          if (!ok) {
            try {
              const r2 = await fetch(API_CUSTOMER_BY_PHONE + encodeURIComponent(phone), {
                signal: ctrl.signal
              });
    ...
    document.addEventListener('DOMContentLoaded', async () => {
      ...
      countSel.addEventListener('change', async () => {
        await generateOrderForms();
      });
    ...
    async function handleOrderModeChange(order) {
      ...
      // 1️⃣ Chỉ ẩn ĐẠI LÝ
      const hideAgency = [
        'lazada', 'shopee', 'tiktok', 'shopee_risoli',
        'hurom', 'bohemia', 'omada'
      ].includes(mode);

      // 2️⃣ Chỉ ẩn SĐT KHÁCH cho marketplace
      const hideCustomerPhone = [
        'lazada', 'shopee', 'tiktok', 'shopee_risoli'
      ].includes(mode);

      // Đại lý
      [agencyNameGroup, agencyPhoneGroup]
      .forEach(g => g && g.classList.toggle('d-none', hideAgency));

      // Khách hàng
      if (custPhoneGroup) {
        custPhoneGroup.classList.toggle('d-none', hideCustomerPhone);
      }
      ...
      // HUROM / BOHEMIA / OMADA → auto code + prefix riêng
      if (ORDER_PREFIX_MAP[mode]) {
        oc1.value = warehouseLabel;
        oc1.readOnly = true;

        oc2.value = await generateUniqueOrderCode(ORDER_PREFIX_MAP[mode]);
        oc2.readOnly = true;
      }
      ...
      // refresh lại select sản phẩm
      const list = document.getElementById(`product_list_${order}`);
      if (list) {
        list.querySelectorAll('.product-select').forEach(sel => {
          const current = sel.value;

          refreshProductSelect(sel, mode);
    ...
    (() => {
      ...
      function openPriceDetail() {
        ...
        if (!c) return;
        const bsCol = bootstrap.Collapse.getOrCreateInstance(c, {
          toggle: false
        });
        bsCol.show();
      }
      ...
      const steps = [{
          el: '#order_count',
          title: 'Chọn số đơn',
          content: 'Chọn nhanh số lượng đơn cần tạo (mặc định 1).',
          placement: 'bottom'
        },
        {
          el: '#order_mode_1',
          title: 'Loại đơn hàng',
          content: 'Chọn kênh/loại đơn. Một số loại sẽ bật ô đơn vị vận chuyển riêng.',
          placement: 'bottom',
          prepare: ensureOrder1AndFirstProduct
        },
        {
          el: '#order_code1_1',
          title: 'Mã đơn vị vận chuyển',
          content: 'Nhập đúng mã ĐVVC (GHTK/Viettel Post/...).',
          placement: 'bottom'
        },
        {
          el: '#order_code2_1',
          title: 'Mã đơn hàng',
          content: 'Hệ thống tự kiểm tra trùng và gợi ý mã mới nếu cần.',
          placement: 'bottom'
        },
        {
          el: '#customer_phone_1',
          title: 'SĐT khách hàng',
          content: 'Nhập đủ 10 số. Hệ thống tự điền tên/địa chỉ/địa giới nếu đã có.',
          placement: 'bottom'
        },
        {
          el: '#discount_code_1',
          title: 'Mã khuyến mại',
          content: 'Chọn nhanh hoặc nhập “Khác” để gõ số tiền giảm.',
          placement: 'bottom'
        },
        {
          el: 'button[data-action="add-product"][data-order="1"]',
          title: 'Thêm sản phẩm',
          content: 'Nhấn để thêm sản phẩm vào đơn.',
          placement: 'left',
          prepare: ensureOrder1AndFirstProduct
        },
        {
          el: '#product_name_1_1',
          title: 'Chọn sản phẩm',
          content: 'Giá đã gồm VAT (Giá sau thuế).',
          placement: 'bottom',
          prepare: ensureOrder1AndFirstProduct
        },
        {
          el: '#quantity_1_1',
          title: 'Số lượng',
          content: 'Nhập số lượng; có cảnh báo nếu lớn bất thường.',
          placement: 'bottom'
        },
        {
          el: '#price_1_1',
          title: 'Giá sau thuế',
          content: 'Tự động tính; mở “chi tiết giá” để xem VAT.',
          placement: 'bottom',
          prepare: () => {
            ensureOrder1AndFirstProduct();
            openPriceDetail();
          }
        },
        {
          el: '.fab-submit .fab-btn',
          title: 'Gửi đơn để quét QR',
          content: 'Nhấn để chuyển sang bước quét QR bảo hành.',
          placement: 'left'
        }
      ];
